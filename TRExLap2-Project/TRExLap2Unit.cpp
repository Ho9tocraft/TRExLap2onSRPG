...
TRExLap2Unit::TRExLap2Unit() : UnitID(u8""), UnitTags({}), UnitImagePath(u8""), UnitBGMPath(u8""), UnitBGMPriority(0), UnitNameJa(u8""), UnitNameEn(u8""),
UnitDescJa({}), UnitDescEn({}), unitHP(0), unitMP(0), unitDEF(0), unitMOB(0), unitSIG(0), unitMVR(0), unitMVT({}), unitADP({}),
unitPSL(TRExLap2PilotPersonalityType::Normal), unitGND(TRExLap2PilotGenderType::Unknown), pilotMEL(0), pilotRNG(0), pilotMAG(0),
pilotDEX(0), pilotDEF(0), pilotAVD(0), pilotRST(0), pilotACC(0), pilotSPP(0), syncedUnit({}),
eikonicSlotCount(0), weaponCostLimit(0), carryableItemLimit(0), unitModifyType(TRExLap2ModifyType::COMMON),
unitEffects({}), unitSkills({}), unitSpiritualCommands({})
{
	// なにもしないデフォルトコンストラクタ
}
...
void TRExLap2IngameUnit::setMorale(std::int64_t toMorale)
{
	if ( toMorale < this->currentMinMorale ) toMorale = this->currentMinMorale;
	if ( toMorale > this->currentMaxMorale ) toMorale = this->currentMaxMorale;
	this->currentMorale = toMorale;
}

TRExLap2IngameUnit::TRExLap2IngameUnit(const TRExLap2Unit& baseData) : TRExLap2Unit(baseData),
level(1LL), experience(0LL), currentMorale(100LL), currentMaxMorale(150LL), currentMinMorale(50LL)
{
	// ランダムデバイスを使って、Xorshift1024ssのシードを生成する(本初期化)。
	std::random_device rd;
	std::uint64_t seed = (static_cast<std::uint64_t>(rd()) << 32) | rd();
	this->randomGenerator = Xorshift1024ss(seed);

	// その他初期化(固定)
	this->level = 1LL;
	this->currentMaxHP = this->unitHP;
	this->currentMaxMP = this->unitMP;
	this->currentMaxSP = this->pilotSPP;
	this->currentDEF = this->unitDEF;
	this->currentSIG = this->unitSIG;
	this->currentMOB = this->unitMOB;
	this->currentMVR = this->unitMVR;
	this->currentPilotMEL = this->pilotMEL;
	this->currentPilotRNG = this->pilotRNG;
	this->currentPilotMAG = this->pilotMAG;
	this->currentPilotDEX = this->pilotDEX;
	this->currentPilotDEF = this->pilotDEF;
	this->currentPilotAVD = this->pilotAVD;
	this->currentPilotRST = this->pilotRST;
	this->currentPilotACC = this->pilotACC;
	this->currentHP = this->currentMaxHP;
	this->currentMP = this->currentMaxMP;
	this->currentSP = this->currentMaxSP;
	this->currentBuffedDEF = this->currentDEF;
	this->currentBuffedSIG = this->currentSIG;
	this->currentBuffedMOB = this->currentMOB;
	this->currentBuffedMVR = this->currentMVR;
	this->currentBuffedPilotMEL = this->currentPilotMEL;
	this->currentBuffedPilotRNG = this->currentPilotRNG;
	this->currentBuffedPilotMAG = this->currentPilotMAG;
	this->currentBuffedPilotDEX = this->currentPilotDEX;
	this->currentBuffedPilotDEF = this->currentPilotDEF;
	this->currentBuffedPilotAVD = this->currentPilotAVD;
	this->currentBuffedPilotRST = this->currentPilotRST;
	this->currentBuffedPilotACC = this->currentPilotACC;
}
...
std::int64_t TRExLap2IngameUnit::getCurrentMorale() const
{
	return this->currentMorale;
}

bool TRExLap2IngameUnit::checkMoraleCond(std::int64_t cMoraleValue, bool isUnder) const
{
	// Underは未満
	return isUnder ? ( this->currentMorale < cMoraleValue ) : ( this->currentMorale >= cMoraleValue );
}

std::uint64_t TRExLap2IngameUnit::getRandomRoller(std::uniform_int_distribution<std::uint64_t> dist)
{
	return dist(this->randomGenerator);
}
...
void TRExLap2IngameUnit::increaseMorale(std::int64_t amount)
{
	if ( amount == 0LL ) return; // 必要がないので即リターン
	this->setMorale(this->currentMorale + amount);
}
...
void TRExLap2IngameUnit::armEffectsAndSkillsExpiringTimer(std::u8string gainId)
{
	// ないならまずゼロ初期化
	if (!this->unitExpiringTimerFlags.contains(gainId)) {
		this->unitExpiringTimerFlags.insert_or_assign(gainId, false);
	}

	// トグルスイッチ化 (終了条件達成でfalseトグル)
	this->unitExpiringTimerFlags.insert_or_assign(gainId, !this->unitExpiringTimerFlags.at(gainId));
}
...
void TRExLap2IngameUnit::approveWeaponRangeFixed(std::u8string gainId, std::int64_t fixedValue)
{
	this->weaponRangeFixed.insert_or_assign(gainId, fixedValue);
}

void TRExLap2IngameUnit::approveWeaponMeleeRangeFixed(std::u8string gainId, std::int64_t fixedValue)
{
	this->weaponMeleeRangeFixed.insert_or_assign(gainId, fixedValue);
}

void TRExLap2IngameUnit::approveWeaponRangedRangeFixed(std::u8string gainId, std::int64_t fixedValue)
{
	this->weaponRangedRangeFixed.insert_or_assign(gainId, fixedValue);
}

void TRExLap2IngameUnit::approveWeaponMagicRangeFixed(std::u8string gainId, std::int64_t fixedValue)
{
	this->weaponMagicRangeFixed.insert_or_assign(gainId, fixedValue);
}

void TRExLap2IngameUnit::approveWeaponAccuracyFixed(std::u8string gainId, std::int64_t fixedValue)
{
	this->weaponAccuracyFixed.insert_or_assign(gainId, fixedValue);
}

void TRExLap2IngameUnit::approveWeaponMeleeAccuracyFixed(std::u8string gainId, std::int64_t fixedValue)
{
	this->weaponMeleeAccuracyFixed.insert_or_assign(gainId, fixedValue);
}

void TRExLap2IngameUnit::approveWeaponRangedAccuracyFixed(std::u8string gainId, std::int64_t fixedValue)
{
	this->weaponRangedAccuracyFixed.insert_or_assign(gainId, fixedValue);
}

void TRExLap2IngameUnit::approveWeaponMagicAccuracyFixed(std::u8string gainId, std::int64_t fixedValue)
{
	this->weaponMagicAccuracyFixed.insert_or_assign(gainId, fixedValue);
}

void TRExLap2IngameUnit::approveWeaponCriticalFixed(std::u8string gainId, std::int64_t fixedValue)
{
	this->weaponCriticalFixed.insert_or_assign(gainId, fixedValue);
}

void TRExLap2IngameUnit::approveWeaponMeleeCriticalFixed(std::u8string gainId, std::int64_t fixedValue)
{
	this->weaponMeleeCriticalFixed.insert_or_assign(gainId, fixedValue);
}

void TRExLap2IngameUnit::approveWeaponRangedCriticalFixed(std::u8string gainId, std::int64_t fixedValue)
{
	this->weaponRangedCriticalFixed.insert_or_assign(gainId, fixedValue);
}

void TRExLap2IngameUnit::approveWeaponMagicCriticalFixed(std::u8string gainId, std::int64_t fixedValue)
{
	this->weaponMagicCriticalFixed.insert_or_assign(gainId, fixedValue);
}

void TRExLap2IngameUnit::approveEvasionRateFixed(std::u8string gainId, std::int64_t fixedValue)
{
	this->evasionRateFixed.insert_or_assign(gainId, fixedValue);
}

void TRExLap2IngameUnit::approveMeleeEvasionRateFixed(std::u8string gainId, std::int64_t fixedValue)
{
	this->meleeEvasionRateFixed.insert_or_assign(gainId, fixedValue);
}

void TRExLap2IngameUnit::approveRangedEvasionRateFixed(std::u8string gainId, std::int64_t fixedValue)
{
	this->rangedEvasionRateFixed.insert_or_assign(gainId, fixedValue);
}

void TRExLap2IngameUnit::approveMagicEvasionRateFixed(std::u8string gainId, std::int64_t fixedValue)
{
	this->magicEvasionRateFixed.insert_or_assign(gainId, fixedValue);
}
